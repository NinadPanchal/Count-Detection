...
export default function AlertPanel({ alerts, isConnected }: AlertPanelProps) {
  ...
  return (
    <SpotlightCard className="flex flex-col h-full flex-1 min-h-[300px]">
      {/* Header */}
      <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--color-border)] z-10 bg-[var(--color-bg-base)]">
      ...
      {/* Alert List */}
      <div ref={containerRef} className="flex-1 overflow-y-auto relative z-10 bg-[var(--color-bg-base)]/50">
        {visibleAlerts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center h-full">
            ...
            <p className="text-xs font-medium text-[var(--color-text-secondary)] tracking-wider uppercase">System Idle</p>
            <p className="text-[10px] text-[var(--color-text-tertiary)] mt-1">Awaiting events</p>
          </div>
        ) : (
          <div className="flex flex-col">
            {visibleAlerts.map((alert, index) => (
              <div
                key={alert.id}
                className={`animate-slide-in-bottom flex items-start gap-3 px-5 py-3.5 border-b border-[var(--color-border)] hover:bg-[var(--color-bg-hover)]/80 transition-colors ${
                  alert.type === "dispatch" ? "bg-[var(--color-critical-muted)]/40" : ""
                }`}
                ...
                <div className="flex-1 min-w-0">
                  <div className={`text-xs leading-relaxed ${
                    ...
                    {alert.severity === "critical" || alert.severity === "warning" ? (
                      <DecryptedText text={alert.message} animateOn="onMount" />
                    ) : (
                      alert.message
                    )}
                  </div>
